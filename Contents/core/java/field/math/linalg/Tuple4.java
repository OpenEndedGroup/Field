 ...
 * 
 */
public abstract class Tuple4 implements java.io.Serializable, Cloneable {

	static long serialVersionUID = 7068460319248845763L;
	 ...
	 * @return this (mutated)
	 */
	public Tuple4 set(float x, float y, float z, float w) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.w = w;
	 ...
	 * @return this (mutated)
	 */
	public Tuple4 add(Tuple4 t1, Tuple4 t2) {
		this.x = t1.x + t2.x;
		this.y = t1.y + t2.y;
		this.z = t1.z + t2.z;
		this.w = t1.w + t2.w;
	 ...
	 * @return this (mutated)
	 */
	public Tuple4 sub(Tuple4 t1, Tuple4 t2) {
		this.x = t1.x - t2.x;
		this.y = t1.y - t2.y;
		this.z = t1.z - t2.z;
		this.w = t1.w - t2.w;
	 ...
	 * @return
	 */
	public Tuple4 negate(Tuple4 t1) {
		this.x = -t1.x;
		this.y = -t1.y;
		this.z = -t1.z;
		this.w = -t1.w;
	 ...
	 * @return
	 */
	public Tuple4 scale(float s, Tuple4 t1) {
		this.x = s * t1.x;
		this.y = s * t1.y;
		this.z = s * t1.z;
		this.w = s * t1.w;
	 ...
	 * @return
	 */
	public Tuple4 scaleAdd(float s, Tuple4 t1, Tuple4 t2) {
		this.x = s * t1.x + t2.x;
		this.y = s * t1.y + t2.y;
		this.z = s * t1.z + t2.z;
		this.w = s * t1.w + t2.w;
	 ...
	 * @return
	 */
	public Tuple4 scaleAdd(float s, Tuple4 t1) {
	 ...
	 * @return true or false
	 */
	public boolean epsilonEquals(Tuple4 t1, float epsilon) {
		float diff;
	...
	/**
	 * Returns a hash code value based on the data values in this object. Two different Tuple4f objects with identical data values (i.e., Tuple4f.equals returns true) will return the same hash code value. Two objects with different data members may return the same hash value, although this is not likely.
	 * 
	 ...
	 */
	public Tuple4 clamp(float min, float max, Tuple4 t) {
		if (t.x > max) {
			x = max;
		} else if (t.x < min) {
			x = min;
	 ...
	 * @return
	 */
	public Tuple4 clampMin(float min, Tuple4 t) {
		if (t.x < min) {
			x = min;
		} else {
			x = t.x;
	 ...
	 * @return
	 */
	public Tuple4 clampMax(float max, Tuple4 t) {
		if (t.x > max) {
			x = max;
		} else {
			x = t.x;
	 ...
	 * @return
	 */
	public Tuple4 absolute(Tuple4 t) {
		x = Math.abs(t.x);
		y = Math.abs(t.y);
		z = Math.abs(t.z);
		w = Math.abs(t.w);
	 ...
	 * @return
	 */
	public Tuple4 clamp(float min, float max) {
		if (x > max) {
			x = max;
		} else if (x < min) {
			x = min;
	 ...
	 * @return
	 */
	public Tuple4 clampMin(float min) {
		if (x < min)
			x = min;
		if (y < min)
			y = min;
	 ...
	 * @return
	 */
	public Tuple4 clampMax(float max) {
		if (x > max)
			x = max;
		if (y > max)
			y = max;
	 ...
	 * @return
	 */
	public Tuple4 interpolate(Tuple4 t1, Tuple4 t2, float alpha) {
	 ...
	 * @return
	 */
	public Tuple4 interpolate(Tuple4 t1, float alpha) {
