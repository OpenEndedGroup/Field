...
/**
 * A float precision floating point 3 by 3 matrix. Primarily to support 3D rotations.
 *
 */
public class Matrix3 implements java.io.Serializable, Cloneable {

	// float[] tmp = new float[9]; // scratch matrix
	// float[] tmp_rot = new float[9]; // scratch matrix
	// float[] tmp_scale = new float[3]; // scratch matrix
	private static final float EPS = 1.110223024E-16f;

	private static final float ERR_EPS = 1.0E-8f;

	private static float xin, yin, zin, xout, yout, zout;

	// Compatible with 1.1
	static final long serialVersionUID = 6837536777072402710L;

	public static double max3(double[] values) {
	...
	private static final boolean almostEqual(float a, float b) {
		...
		final float EPSILON_ABSOLUTE = 1.0e-6f;
		final float EPSILON_RELATIVE = 1.0e-4f;
		float diff = Math.abs(a - b);
		float absA = Math.abs(a);
		float absB = Math.abs(b);
		float max = (absA >= absB) ? absA : absB;
	...
	/**
	 * Given a 3x3 array "matrix0", this function replaces it with the LU decomposition of a row-wise permutation of itself. The input parameters are "matrix0" and "dimen". The array "matrix0" is also an output parameter. The vector "row_perm[3]" is an output parameter that contains the row permutations resulting from partial pivoting. The output parameter "even_row_xchg" is 1 when the number of row exchanges is even, or -1 otherwise. Assumes data type is always float.
	 *
	...
	static boolean luDecomposition(float[] matrix0, int[] row_perm) {
...

		{
			...
			for (j = 0; j < 3; j++) {
				...
				// Is the matrix singular
				if (matrix0[(mtx + (3 * j) + j)] == 0.0f) { return false; }

				// Divide elements of lower diagonal matrix L by pivot
				if (j != (3 - 1)) {
					temp = 1.0f / (matrix0[(mtx + (3 * j) + j)]);
					target = mtx + (3 * (j + 1)) + j;
					i = 2 - j;
					while (i-- != 0) {
						matrix0[target] *= temp;
	...
	static void mat_mul(float[] m1, float[] m2, float[] m3) {
		int i;
	...
	static void print_det(float[] mat) {
		float det;
	...
	static void transpose_mat(float[] in, float[] out) {
		out[0] = in[0];
		out[1] = in[3];
		out[2] = in[6];
	 ...
	 *                        the threshold value
	 */
	public boolean epsilonEquals(Matrix3 m1, float epsilon) {
		float diff;
	...
	public Vector3 getScale(Vector3 localScale) {
		if (localScale == null) localScale = new Vector3();
		float[] tmp_scale = new float[3]; // scratch matrix
		float[] tmp_rot = new float[9]; // scratch matrix
		getScaleRotate(tmp_scale, tmp_rot);
		localScale.set(tmp_scale);
		return localScale;
	}
	...
	public final void normalizeCP() {
		float mag = (float) (1.0f / Math.sqrt(m00 * m00 + m10 * m10 + m20 * m20));
		m00 = m00 * mag;
		m10 = m10 * mag;
		m20 = m20 * mag;

		mag = (float) (1.0f / Math.sqrt(m01 * m01 + m11 * m11 + m21 * m21));
		m01 = m01 * mag;
		m11 = m11 * mag;
		m21 = m21 * mag;
	...
	public final void normalizeCP(Matrix3 m1) {
		float mag = (float) (1.0f / Math.sqrt(m1.m00 * m1.m00 + m1.m10 * m1.m10 + m1.m20 * m1.m20));
		m00 = m1.m00 * mag;
		m10 = m1.m10 * mag;
		m20 = m1.m20 * mag;

		mag = (float) (1.0f / Math.sqrt(m1.m01 * m1.m01 + m1.m11 * m1.m11 + m1.m21 * m1.m21));
		m01 = m1.m01 * mag;
		m11 = m1.m11 * mag;
		m21 = m1.m21 * mag;
	...
	/**
	 * Sets the value of this matrix to the float value of the Matrix3f argument.
	 *
	 * @param m1
	 *                        the matrix3d to be converted to float
	 */
	...
	public final void transformH(Vector2 t) {
		float x, y, z;
		x = m00 * t.x + m01 * t.y + m02 ;
		y = m10 * t.x + m11 * t.y + m12 ;
		z = m20 * t.x + m21 * t.y + m22 ;
		t.set(x/z, y/z);
	}
	 ...
	 */
	public final void transform(Vector3 t, Vector3 result) {
		float x, y, z;
		x = m00 * t.x + m01 * t.y + m02 * t.z;
		y = m10 * t.x + m11 * t.y + m12 * t.z;
	 ...
	 */
	private final void invertGeneral(Matrix3 m1) {
	 ...
	 */
	final void getScaleRotate(float scales[], float rots[]) {
		...
		double[] tScales = new double[scales.length];
		double[] tRots = new double[rots.length];

		compute_svd(tmp, tScales, tRots);
		for (int i = 0; i < scales.length; i++)
			scales[i] = (float) tScales[i];
		for (int i = 0; i < rots.length; i++)
			rots[i] = (float) tRots[i];
