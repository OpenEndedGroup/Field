...
public class CubicInterpolatorDynamic_double<T extends iBlendable<T>> implements Serializable{
	...
	public class Sample implements Serializable {
		...
		public Sample(T data, double time) {
			this.data = data;
			this.time = time;
	...
	public CubicInterpolatorDynamic_double() {
	}


	public CubicInterpolatorDynamic_double<T> copy() {
	...
	public T debugGet(double alpha) {
		if (getNumSamples() == 0)
			return null;
	...
	public CubicInterpolatorDynamic_double<T> extrapolate() {
		extrapolation = true;
		return this;
	}

	public int findSampleIndexAfter(double from) {
		temp.time = from;
	...
	public int findSampleIndexBefore(double from) {
		temp.time = from;
		int n = Collections.binarySearch(samples, temp, comparator);
		if (n < 0)
			n = -n - 2;
	...
	public boolean getValue(double time, T value) {
		if (samples.size() < 1)
			return false;
	...
	public CubicInterpolatorDynamic_double<T> setLinear(boolean linear) {
		this.linear = linear;
		return this;
	}
