...
public class ColorDialog extends JDialog implements ChangeListener {
	...
	public ColorDialog(Color cc, Frame owner, Point pointOnScreen) {
		super(owner, "Choose Color");
		getRootPane().putClientProperty("Window.style", "small");
		this.setUndecorated(true);
		setAlwaysOnTop(true);
		setBackground(new Color(0.75f, 0.75f, 0.75f, 1f));
		...
		cancelButton.putClientProperty("Quaqua.Button.style", "square");
		okButton.putClientProperty("Quaqua.Button.style", "square");
		
		cancelButton.putClientProperty("JComponent.sizeVariant", "small");
		okButton.putClientProperty("JComponent.sizeVariant", "small");
		...
		this.getRootPane().putClientProperty("Window.alpha", 0.99f);
		...
		setBounds(pointOnScreen.x, pointOnScreen.y, 400, 360);
		pack();
...
//		Quaqua15TigerLookAndFeelField.paintBackgroundGradient(g, bounds, this.isFocused(), this.getHeight());
//		
		super.paint(g);
	}
	...
	public void setHSBA(float h, float s, float b, float a) {
		if (hue == h && saturation == s && brightness == b && alpha == a)
			return;
		hue = clamp(h);
		saturation = clamp(s);
		brightness = clamp(b);
	...
	public class ColorPanel extends JPanel implements ChangeListener {
		...
		public ColorPanel(ColorComponent colorComponent) {
			...
			setPreferredSize(d);
			setMaximumSize(d);
			JLabel label = new JLabel(colorComponent.toString().toLowerCase(), JLabel.RIGHT);
			...
			slider.setBorder(new EmptyBorder(12,0,0,0));
			add(slider);
	...
	public class ColorSlider extends JComponent {
		...
		private void drawAlpha(Graphics g) {
			...
			for (int i = 0; i < width; i++) {
				Color c = new Color(0.65f,0.65f,0.65f,1f);
				g.setColor(c);
				g.fillRect(i + HALF_WIDTH_OFFSET, 0, 1, r.height - HEIGHT_OFFSET);				
				...
				g.setColor(c);
				if ((i/1)%20<10)
					g.fillRect(i + HALF_WIDTH_OFFSET, 0, 1, (r.height - HEIGHT_OFFSET)/2);
				else
					g.fillRect(i + HALF_WIDTH_OFFSET, (r.height - HEIGHT_OFFSET)/2, 1,(r.height - HEIGHT_OFFSET)/2);
			}			
