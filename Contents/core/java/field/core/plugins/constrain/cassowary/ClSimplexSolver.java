...
public class ClSimplexSolver extends ClTableau {
	...
	public ClSimplexSolver() {
		_stayMinusErrorVars = new Vector<ClSlackVariable>();
		_stayPlusErrorVars = new Vector<ClSlackVariable>();
		...
		_editVarMap = new Hashtable<ClVariable, ClEditInfo>();
		_slackCounter = 0;
	...
	public final ClSimplexSolver addBounds(ClAbstractVariable v, double lower, double upper) throws ExCLRequiredFailure, ExCLInternalError {
	...
	public final ClSimplexSolver removeEditVarsTo(int n) throws ExCLInternalError {
		try {
			for (Enumeration<ClVariable> e = _editVarMap.keys(); e.hasMoreElements();) {
				ClVariable v = e.nextElement();
				ClEditInfo cei = _editVarMap.get(v);
				if (cei.Index() >= n) {
					removeEditVar(v);
				}
			}
			_assert(_editVarMap.size() == n, "_editVarMap.size() == n");
			return this;
	...
	public final ClSimplexSolver addPointStay(ClVariable vx, ClVariable vy, double weight) throws ExCLRequiredFailure, ExCLInternalError {
	...
	public final ClSimplexSolver addStay(ClVariable v, ClStrength strength, double weight) throws ExCLRequiredFailure, ExCLInternalError {
	...
	public final ClSimplexSolver removeConstraint(ClConstraint cn) throws ExCLConstraintNotFound, ExCLInternalError {
		if (fTraceOn)
		...
		Set eVars = _errorVars.get(cn);
		if (fTraceOn)
			traceprint("eVars == " + eVars);
		if (eVars != null) {
			for (Enumeration<Object> e = eVars.elements(); e.hasMoreElements();) {
	...
	public final void resolve(Vector<ClDouble> newEditConstants) throws ExCLInternalError {
		if (fTraceOn)
	...
	public Hashtable<ClConstraint, ClAbstractVariable> getConstraintMap() {
		return _markerVars;
	}
	...
	protected final void addWithArtificialVariable(ClLinearExpression expr) throws ExCLRequiredFailure, ExCLInternalError {
		...
		_assert(rowExpression(av) == null, "rowExpression(av) == null");
		removeColumn(av);
		removeRow(az);
	}
	...
	protected final void deltaEditConstant(double delta, ClAbstractVariable plusErrorVar, ClAbstractVariable minusErrorVar) {
		...
		Set columnVars = _columns.get(minusErrorVar);
		for (Enumeration<Object> e = columnVars.elements(); e.hasMoreElements();) {
			final ClAbstractVariable basicVar = (ClAbstractVariable) e.nextElement();
			ClLinearExpression expr = rowExpression(basicVar);
			//_assert(expr != null, "expr != null" );
	...
	protected final void dualOptimize() throws ExCLInternalError {
		...
		while (!_infeasibleRows.isEmpty()) {
			ClAbstractVariable exitVar = (ClAbstractVariable) _infeasibleRows.elements().nextElement();
	...
	protected final ClLinearExpression newExpression(ClConstraint cn, Vector<ClSlackVariable> eplus_eminus, ClDouble prevEConstant) {
		if (fTraceOn)
	...
	protected final void setExternalVariables() {
		...
		if (fTraceOn)
			traceprint(this.toString());
		for (Enumeration<Object> e = _externalParametricVars.elements(); e.hasMoreElements();) {
		...
		for (Enumeration<Object> e = _externalRows.elements(); e.hasMoreElements();) {
			ClVariable v = (ClVariable) e.nextElement();
