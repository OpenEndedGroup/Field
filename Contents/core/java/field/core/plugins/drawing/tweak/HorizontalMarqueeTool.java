...
public class HorizontalMarqueeTool implements iSimpleSelectionTool {
	...
	private void doSelectionNow() {
		if (downAt == null || upAt == null) return;
	...
	public void paint() {
		...
		//marqueLine.getProperties().put(iLinearGraphicsContext.strokeType, new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1, new float[] { 5, 5 }, 0));
		...
		marqueLine.getInput().moveTo((float) GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2()).x, downAt.y);
		marqueLine.getInput().lineTo((float) (GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(5000,0)).x), downAt.y);
		marqueLine.getInput().lineTo((float) (GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(5000,0)).x), upAt.y);
		marqueLine.getInput().lineTo((float) (GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0,0)).x), upAt.y);
		marqueLine.getInput().lineTo((float) (GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0,0)).x), downAt.y);
