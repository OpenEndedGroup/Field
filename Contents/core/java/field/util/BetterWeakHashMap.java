...
/**
 * handles the case when the value refers to the key, but only when the value implements iWeakHashMapKey interface
 *
 * @author marc
 */
...
public class BetterWeakHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
	...
	static public class BaseWeakHashMapKey implements iWeakHashMapKey {
		...
		public Object getWHMKValue(Object inside) {
			return map.get(inside);
			// if (onlyOneFrom==inside)
			// {
			// return onlyOne;
			// }
	...
	public interface iWeakHashMapKey {
	...
	private static class Entry extends WeakReference implements Map.Entry {
		...
		public Object getValue() {
			...
			if (get() instanceof iWeakHashMapKey) return ((iWeakHashMapKey) get()).getWHMKValue(this);
			return value;
		}
		...
		public Object setValue(Object newValue) {
			...
			if (get() instanceof iWeakHashMapKey) {
				((iWeakHashMapKey) get()).setWHMKValue(this, newValue);
			} else
				value = newValue;
			return oldValue;
		}
	...
	// {
	// Object onlyOne = null;
	// Object onlyOneFrom = null;
	// IdentityHashMap map = null;
	//
	// public void setWHMKValue(Object inside, Object to) {
	// // map.put(inside, to);
	//
	// if (onlyOne == null && onlyOneFrom==null)
	// {
	// onlyOne = to;
	// onlyOneFrom = inside;
	// return;
	...
	private abstract class HashIterator implements Iterator {
		...
		public void remove() {
			...
			BetterWeakHashMap.this.remove(currentKey);
	...
	private class KeySet extends AbstractSet {
		...
		public boolean remove(Object o) {
			if (containsKey(o)) {
				BetterWeakHashMap.this.remove(o);
				return true;
			} else
				return false;
		}
	...
	static {
		Launcher.getLauncher().registerUpdateable(new iUpdateable(){
			...
			public void update() {
				Iterator<WeakReference<BetterWeakHashMap>> i = weakness.iterator();
				while (i.hasNext()) {
					WeakReference<BetterWeakHashMap> m = i.next();
					BetterWeakHashMap map = m.get();
					if (map == null) {
						i.remove();
					} else {
	...
	/**
	 * Constructs a new, empty <tt>WeakHashMap</tt> with the default initial capacity (16) and the default load factor (0.75).
	 */
	public BetterWeakHashMap() {
		this.loadFactor = DEFAULT_LOAD_FACTOR;
		threshold = (DEFAULT_INITIAL_CAPACITY);
		table = new Entry[DEFAULT_INITIAL_CAPACITY];
		weakness.add(new WeakReference(this));
		//new Exception().printStackTrace();
	}

	/**
	 * Constructs a new, empty <tt>WeakHashMap</tt> with the given initial capacity and the default load factor, which is <tt>0.75</tt>.
	 *
	 ...
	 */
	public BetterWeakHashMap(int initialCapacity) {
	 ...
	 */
	public BetterWeakHashMap(int initialCapacity, float loadFactor) {
	...
	/**
	 * Constructs a new <tt>WeakHashMap</tt> with the same mappings as the specified <tt>Map</tt>. The <tt>WeakHashMap</tt> is created with default load factor, which is <tt>0.75</tt> and an initial capacity sufficient to hold the mappings in the specified <tt>Map</tt>.
	 *
	 * @param t
	 ...
	 * @since 1.3
	 */
	public BetterWeakHashMap(Map t) {
	 ...
	 * @return previous value associated with specified key, or <tt>null</tt> if there was no mapping for key. A <tt>null</tt> return can also indicate that the HashMap previously associated <tt>null</tt> with the specified key.
	 */
	@Override
	 ...
	 *
	 * If current capacity is MAXIMUM_CAPACITY, this method does not resize the map, but but sets threshold to Integer.MAX_VALUE. This has the effect of preventing future calls.
	 *
	 * @param newCapacity
