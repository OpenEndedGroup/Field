...
public class ExternalHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	public interface iEquality<T> {
		...
		public int hashCodeFor(T o1);
	}

	iEquality<K> equality = new iEquality<K>(){
		public boolean areEqual(K o1, K o2) {
			return o1.equals(o2);
		}

		public int hashCodeFor(K o1) {
			return o1.hashCode();
		}
	};

	public ExternalHashMap<K, V> setEquality(iEquality<K> equality) {
	...
	/**
	 * The number of times this ExternalHashMap has been structurally modified Structural modifications are those that change the number of mappings in the ExternalHashMap or otherwise modify its internal structure (e.g., rehash). This field is used to make iterators on Collection-views of the ExternalHashMap fail-fast. (See ConcurrentModificationException).
	 */
	transient volatile int modCount;

	/**
	 * Constructs an empty <tt>ExternalHashMap</tt> with the specified initial capacity and load factor.
	 * 
	...
	/**
	 * Constructs an empty <tt>ExternalHashMap</tt> with the specified initial capacity and the default load factor (0.75).
	 * 
	 * @param initialCapacity
	 *                        the initial capacity.
	 * @throws IllegalArgumentException
	...
	/**
	 * Constructs an empty <tt>ExternalHashMap</tt> with the default initial capacity (16) and the default load factor (0.75).
	 */
	...
	/**
	 * Constructs a new <tt>ExternalHashMap</tt> with the same mappings as the specified <tt>Map</tt>. The <tt>ExternalHashMap</tt> is created with default load factor (0.75) and an initial capacity sufficient to hold the mappings in the specified <tt>Map</tt>.
	 * 
	 * @param m
	...
	/**
	 * Initialization hook for subclasses. This method is called in all constructors and pseudo-constructors (clone, readObject) after ExternalHashMap has been initialized but before any entries have been inserted. (In the absence of this method, readObject would require explicit knowledge of subclasses.)
	 */
	void init() {
	}
	...
	/**
	 * Returns a hash value for the specified object. In addition to the object's own hashCode, this method applies a "supplemental hash function," which defends against poor quality hash functions. This is critical because ExternalHashMap uses power-of two length hash tables.
	 * <p>
	 * 
	 ...
	 */
	static int hash(Object x, iEquality e) {
		int h = e.hashCodeFor(x);
	...
	/**
	 * Returns the entry associated with the specified key in the ExternalHashMap. Returns null if the ExternalHashMap contains no mapping for this key.
	 */
	 ...
	 * @return previous value associated with specified key, or <tt>null</tt> if there was no mapping for key. A <tt>null</tt> return can also indicate that the ExternalHashMap previously associated <tt>null</tt> with the specified key.
	 */
	...
	/**
	 * Removes and returns the entry associated with the specified key in the ExternalHashMap. Returns null if the ExternalHashMap contains no mapping for this key.
	 */
	...
	/**
	 * Returns a shallow copySource of this <tt>ExternalHashMap</tt> instance: the keys and values themselves are not cloned.
	 * 
	 * @return a shallow copySource of this map.
	 */
	public Object clone() {
	...
	static class Entry<K, V> implements Map.Entry<K, V> {
		...
		public K getKey() {
			return ExternalHashMap.<K> unmaskNull(key);
		}
		...
		/**
		 * This method is invoked whenever the value in an entry is overwritten by an invocation of put(k,v) for a key k that's already in the ExternalHashMap.
		 */
	 ...
	 * 
	 * Subclass overrides this to alter the behavior of ExternalHashMap(Map), clone, and readObject.
	 */
	...
	/**
	 * Save the state of the <tt>ExternalHashMap</tt> instance to a stream (i.e., serialize it).
	 * 
	 * @serialData The <i>capacity</i> of the ExternalHashMap (the length of the bucket array) is emitted (int), followed by the <i>size</i> of the ExternalHashMap (the number of key-value mappings), followed by the key (Object) and value (Object) for each key-value mapping represented by the ExternalHashMap The key-value mappings are emitted in the order that they are returned by <tt>entrySet().iterator()</tt>.
	 * 
	 */
	...
	/**
	 * Reconstitute the <tt>ExternalHashMap</tt> instance from a stream (i.e., deserialize it).
	 */
	private void readObject(java.io.ObjectInputStream s) throws IOException, ClassNotFoundException {
		...
		// Read the keys and values, and put the mappings in the ExternalHashMap
