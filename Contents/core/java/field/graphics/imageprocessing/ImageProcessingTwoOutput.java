 ...
 */
@Woven
public class ImageProcessingTwoOutput implements iImageProcessor {
	...
	public ImageProcessingTwoOutput(iProvider<Integer>[] fboInput, int width, int height, boolean useRect, boolean genMipmap, boolean useFloat) {
	...
	public ImageProcessingTwoOutput(final iHasFBO[] fboInput, int width, int height, boolean useRect, boolean genMipmap, boolean useFloat) {
	...
	public ImageProcessingTwoOutput setClearColor(Vector4 clearColor, Vector4 clearColor2) {
		this.clearColor = clearColor;
		this.clearColor2 = clearColor2;
		return this;
	}

	IntBuffer color01 = ByteBuffer.allocateDirect(4 * 2).order(ByteOrder.nativeOrder()).asIntBuffer().put(new int[] { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 });
	...
	public void initialize() {
		...
		glTexImage2D(useRect ? GL_TEXTURE_RECTANGLE : GL_TEXTURE_2D, 0, useFloat ? GL_RGBA16F : GL_RGBA, width, height, 0, GL_RGBA, useFloat ? GL11.GL_FLOAT : GL_UNSIGNED_INT_8_8_8_8_REV, (ByteBuffer) null);
	...
	public BasicUtilities.TwoPassElement getOutputElement(final int num, final boolean leftOnly) {
		return new BasicUtilities.TwoPassElement("", StandardPass.preRender, StandardPass.postRender) {
