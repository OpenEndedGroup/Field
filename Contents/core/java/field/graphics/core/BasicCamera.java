	...
	static public class Projector implements Serializable{
		...
		public void createIntersectionRay(float x, float y, Vector3 rayIntersection1, Vector3 rayIntersection2, float width, float height) {
			...
			synchronized (BasicContextManager.gluLock) {
				GLU.gluUnProject((float) x, (float) y, 0.0f, model, projection, viewport, object1);
				GLU.gluUnProject((float) x, (float) y, 1.0f, model, projection, viewport, object2);
			}
			rayIntersection1.set((float) object1.get(0), (float) object1.get(1), (float) object1.get(2));
			rayIntersection2.set((float) object2.get(0), (float) object2.get(1), (float) object2.get(2));
		}
...
//		glGetFloat(GL_PROJECTION_MATRIX, projectionn);
	...
	public void performPass() {
		...
		synchronized (BasicContextManager.gluLock) {
			...
			if (Base.trace)
				System.out.println(" --------- applied camera <" + this + "> (" + name + ")");
	...
	public Vector3 project3(Vector3 worldPt) {
		...
		// return result
		return new Vector3((float) object1.get(0), (float) object1.get(1), (float) object1.get(2));
	}
	...
	public Vector3 unProject(double x, double y, double[] plane) {
		...
		Vector3 lu = new Vector3((float) nearPtX.x, (float) nearPtX.y, (float) nearPtX.z);
		Vector3 ld = (new Vector3((float) farPtX.x, (float) farPtX.y, (float) farPtX.z)).sub(lu).normalize();
